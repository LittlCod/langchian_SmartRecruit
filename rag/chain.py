...
async def get_rag_chain():
    ...
    # 生成 ANSWER_PROMPT 中的 content 的函数
    async def retrieve_and_format_content(input_dict: dict, config: RunnableConfig) -> str:
        # 查询改写的 chain
        ...
        # 混合检索
        try:
            retrived_docs = await retriever.aget_relevant_documents(query=rewritten_question, params=params)
        except Exception as e:
            logger.error(f"检索器执行失败: {e}", exc_info=True)
            return "检索简历时发生内部错误，请稍后再试。"

        # 格式化
        context = _format_docs(retrived_docs)
        logger.debug(f"为LLM准备的上下文: \n{context}")
        return context


    # 最终的 chain
    rag_chain = ((RunnablePassthrough.assign(context=retrieve_and_format_content))
