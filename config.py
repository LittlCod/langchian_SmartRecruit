...
class Config(BaseModel):
    ...
    # 模型路径
    MODEL_PATH: str = Field(default=os.path.join(os.path.dirname(os.path.abspath(__file__)),"models"), description="本地模型路径")
    ...
    # API 密钥
    DASHSCOPE_API_KEY: str = Field(default=os.getenv("DASHSCOPE_API_KEY", ""), description="DashScope API 密钥")

    # 本地简历路径
    LOCAL_RESUME_DIR: str = Field(default= os.path.join(os.path.dirname(os.path.abspath(__file__)),"data","resume"),
                                  description="本地简历目录")
